#!/usr/bin/env bash
# Bootstrap hcp-oss/kicad from upstream (run OUTSIDE the HCP monorepo).
set -euo pipefail

HCP_OSS_ORG="${HCP_OSS_ORG:-hcp-oss}"
REPO_NAME="kicad"
UPSTREAM_URL="https://gitlab.com/kicad/code/kicad.git"
...
render_template() {
  local src="$1" dest="$2"
  sed \
    -e "s|{{TOOL_NAME}}|KiCad 8.x|g" \
    -e "s|{{REPO_NAME}}|${REPO_NAME}|g" \
    -e "s|{{UPSTREAM_URL}}|${UPSTREAM_URL}|g" \
    -e "s|{{UPSTREAM_LICENSE}}|GPL-3.0|g" \
    -e "s|{{UPSTREAM_DEFAULT_BRANCH}}|${UPSTREAM_BRANCH}|g" \
    -e "s|{{UPSTREAM_GIT_URL}}|${UPSTREAM_URL}|g" \
    -e "s|{{STEWARD_TEAM}}|hcp-oss-stewards|g" \
    -e "s|{{PRIMARY_CLI}}|kicad-cli|g" \
    -e "s|{{VERSION_PIN}}|8.0.x stable|g" \
    -e "s|{{ADAPTER_SUFFIX}}|kicad|g" \
    -e "s|{{EXPORT_FORMATS}}|STEP, Gerber, netlist|g" \
    "${src}" > "${dest}"
}
